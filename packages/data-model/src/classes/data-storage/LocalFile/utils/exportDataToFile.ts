...
export default function exportDataToFile(
  data: any,
  ...
  if (
    !data ||
    typeof data !== "object" ||
    (!Object.keys(data).length && !Array.isArray(data))
  ) {
    console.error(__filename, `Data should be an object or an array`, {
  ...
  try {
    ...
    console.error(
      __filename,
      `UNKNOWN ERROR creating File: "${resolvedOutputFilePath}"`
    ); // log error
    return false;
  } catch (error) {
    console.error(
      __filename,
      `ERROR creating File: "${resolvedOutputFilePath}"`,
      { error }
    ); // log error
    return false;
  }
