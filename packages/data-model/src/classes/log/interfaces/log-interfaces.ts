...
import {
  LogOperationUnion, LogSourceTypeNameUnion, TraitValueTypeUnion
...
export interface iHasLogEvents {
  readonly events: iLogEvent<TraitValueTypeUnion>[];
}

export interface iCanLog {
  log<V>(event: iLogEvent<V>): void;
}
...
// todo relocate
export interface iLogDescriberProps<V> extends iBaseLogEventProps, iHasOldValue<V>, iHasNewValue<V> {
  // describer: () => string;
}

// LOG EVENT PROPS
export interface iBaseLogEventProps   {
  // todo delete, description will be generated by functions
  property: string;
}
export interface iBaseLogEventConstructorProps<V> extends iBaseLogEventProps, iHasOldValue<V>, iHasNewValue<V> {
  describer: (props: iBaseLogEventConstructorProps<V>) => string;
...
export interface iBaseLoggerProps {
  ...
  parentLogHandler: (<V>(event: iLogEvent<V>) => void) | null;
  sourceName: string;
}
...
export interface iLogEvent<V> extends iBaseLogEventProps, iCanDescribe, iHasNewValue<V>, iHasOldValue<V> {
...
export interface iAddLogEvent<V> extends iLogEvent<V>, iHasNewValue<V> {}
// -------------------------------------------------------
// LOG REPORTS
